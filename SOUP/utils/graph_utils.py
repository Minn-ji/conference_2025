...
def get_avg_quiz_score(q: Optional[RecentQuizInfo]) -> Optional[float]:
...
def extract_accuracy_by_topic(q: Optional[RecentQuizInfo]) -> Optional[Dict[str, float]]:
    # 데이터가 없다 가정: 외부에서 전처리해 넣는 게 이상적. 없으면 None.
    return None

def extract_accuracy_by_difficulty(q: Optional[RecentQuizInfo]) -> Optional[Dict[str, float]]:
    # 데이터가 없다 가정: 외부에서 전처리해 넣는 게 이상적. 없으면 None.
    return None
